...
def _ovewrite_named_param(kwargs, param: str, new_value) -> None:
    if param in kwargs:
        if kwargs[param] != new_value:
            raise ValueError(
                f"The parameter '{param}' expected value {new_value} but got "
                f"{kwargs[param]} instead.")
    else:
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.bn1 = norm_layer(num_filters = self.inplanes)
        self.relu = activation()
    ...
    def _make_layer(
        ...
        layers.append(
            block(
                self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, activation, norm_layer
            )
        )
...
def _resnet(
    ...
    **kwargs: Any,
    ...
    if weights is not None:
        raise ValueError('pretrained weight not supported')
